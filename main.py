            ...
            if(pole[otvet] == -1):
                if(xod[clients.index(user)] == 1):
                    ...
                    msg = json.dumps(
                        {
                            'Element': stroka,
                            'KrestikAndNolik': clients.index(user),
                            'pole': pole,
                            "VIN": viner,
                            "playerGames": nickname,
                            "restart": "No"
                        }
                    )
    ...
    def listen(self, user):
        ...
        while is_work:
            try:
                ...
                self.VinnersUser()
                msg = json.dumps(
                    {'Element': "null", 'KrestikAndNolik': "null", 'pole': pole, "VIN": "restart",
                     "playerGames": nickname, "restart": "Yes"}
                )
                self.sender(user, msg)
                is_work = False
